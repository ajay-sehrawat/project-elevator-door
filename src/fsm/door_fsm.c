...
static const char *TAG =
    "FSM"; // Unquie file identifier for logging or output to console
...
typedef struct {
  ...
  uint8_t homing_complete;    // 1 = homing done this session. basically
                              // Claibration Done or not
} fsm_ctx_t;
...
// Motor Stall Timer, Timer is used to detect if motor runs too long ->
// something is wrong ( stall ).
...
static void stall_timer_callback(void *arg) {
  // Called from esp_timer task — post to FSM queue
  ...
  if (s_central_queue_ref != NULL) { // check queue is intialized before posting
    if (dispatcher_post_event(s_central_queue_ref, &stall_evt) !=
        pdTRUE) { // Sends event to dispatcher queue pdTrue = 1 in FreeRtos
      ESP_LOGE(
          TAG,
          "CRITICAL: FSM queue full — stall event lost!"); // If queue full logs
    }
  }
}
...
static void stall_timer_start(void) {
  ...
  uint64_t timeout_us = (uint64_t)MOTOR_STALL_TIMEOUT_MS // 3.6 sec
                        * (100u + CLOCK_DRIFT_MARGIN_PCT) / 100u *
                        1000ULL; // 3.6 * 1.2 as 20% clocl drift
  esp_timer_start_once(
      s_stall_timer,
      timeout_us); // ESP function start timer after timout call
                   // stall_timer_callback function through s_stall_timer
  ESP_LOGD(TAG, "Stall timer started (%lu ms)",
           (unsigned long)(timeout_us / 1000u));
}

static void stall_timer_cancel(void) // Cancel timer if running
{
  esp_timer_stop(
      s_stall_timer); /* Idempotent — safe to call even if not running. */
  ESP_LOGD(TAG, "Stall timer cancelled");
}

// Tranition State to Another state Acquires mutex, updates state, logs,
// releases mutex.
static void fsm_transition(fsm_state_t new_state, fault_code_t fault_code,
                           const system_event_t *trigger) {
  // take Lock to save state Lock FSM context before modifying as multiple tasks
  // can access s_fsm_ctx
  ...
  if (fault_code !=
      FAULT_NONE) { // Not every transition is a fault so conditional
  ...
  if (old_state != new_state) { // To avoid unnecessary logging of same state
    ...
    ESP_LOGI(
        TAG, "State: %d → %d (evt=0x%02X)", (int)old_state, (int)new_state,
        (unsigned)(trigger ? trigger->type : 0)); // Ouputs tranition to console
  }
}
...
static void fsm_enter_fault(fault_code_t code, const system_event_t *trigger) {
  ...
  if (nvs_ret != ESP_OK) { // If write fails output to console
    ESP_LOGE(TAG, "NVS write failed: %s — fault NOT persisted!",
...
// Internal: state name string (for logging), Converts an FSM state (enum value)
...
// Homing Routine
// Drives the door to a known physical position before accepting commands
// (SR-4).

static void fsm_start_homing(void) {
  fsm_transition(FSM_STATE_HOMING, FAULT_NONE,
                 NULL); // Trigger = NULL as internal action of FSM not caused
                        // by external event
...
esp_err_t fsm_init(void) {
  g_fsm_mutex = xSemaphoreCreateMutex(); // Variable declared at start of file
                                         // so global scope
  if (g_fsm_mutex == NULL) {
  ...
  g_fsm_queue = xQueueCreate(
      DISPATCHER_QUEUE_DEPTH,
      sizeof(system_event_t)); //(no of events = 16 , size of each event)
  if (g_fsm_queue == NULL) {
    ESP_LOGE(TAG, "Failed to create FSM queue");
    return ESP_ERR_NO_MEM;
  }

  // Create stall timer, not started yet
...
// Functions that let other parts of the system read FSM data safely without
// corrupting it. Thread Safe way as multiple tasks run in parallel like fsm,
// logger, safety
...
static void fsm_process_event(const system_event_t *evt) {
  ...
  case EVT_CMD_RESET:
    if (cur_state != FSM_STATE_FAULT && emg_lock == EMERGENCY_LOCK_OFF) {
      ...
      ESP_LOGI(TAG, "RESET received in state %s — no action",
  ...
  case EVT_CMD_CLOSE:
    ...
    if (cur_state == FSM_STATE_OPENING || cur_state == FSM_STATE_HOMING ||
...
void fsm_control_task(void *pvParameters) {
  ...
  ESP_LOGI(TAG, "FSM Control task started (Core %d)",
           xPortGetCoreID()); // Prints which CPU core this task is running on.
  ...
  for (;;) {
    ...
    if ((now_ms - last_report_ms) >= STATE_REPORT_INTERVAL_MS) { // 100ms
      ...
      snprintf(state_buf, UART_FRAME_MAX_LEN, "$STATE,STATE=%s,TS=%lu\n",
